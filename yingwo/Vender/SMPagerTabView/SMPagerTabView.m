...
- (void)buildUI {
    ...
    for (int i = 0; i < number; i++) {
        ...
        //改变self.tabMargin的大小，控制button的间距
...
- (UIColor *)tabButtonTitleColorForSelected {
    if (!_tabButtonTitleColorForSelected) {
        _tabButtonTitleColorForSelected = [UIColor colorWithHexString:THEME_COLOR_1];
    }
...
//        self.bodyScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _tabFrameHeight, self.width, self.height - _tabFrameHeight)];
        self.bodyScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height - _tabFrameHeight)];
